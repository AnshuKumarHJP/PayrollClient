...
const FieldValidationRuleForm = () => {
  ...
  useEffect(() => {
    ...
    const rule = data.find((r) => r.Id === Number(decryptedId));
    if (!rule) return;
    let params = [];
    try {
  ...
  const handleSubmit = useCallback(
    ...
    [form, errors, dispatch, toast]
  );

  const onCancel = () => {
    navigate("/fieldValidationRule");
  }
          ...
          <Card >
            ...
            <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1">
                <Label className="text-slate-900 dark:text-white">Rule Code</Label>
                <Input
                  name="RuleCode"
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between w-full">
                ...
                <Button type="button" size="sm" variant="primary" onClick={addParam}>
                  + Add Parameters
                </Button>
              </div>
            </CardHeader>
